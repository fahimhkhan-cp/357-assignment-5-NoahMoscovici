...
void send_request(int fd)
{
   ...
   {
      // use fprintf to print the line to the file (which is the connection)
      fprintf(connection, "%s", line);

      // Read response from the server
      if ((num = getline(&line, &size, connection)) >= 0)
      {
         printf("response from server: %s", line);
      }
      else
      {
         break;
